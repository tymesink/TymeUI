...
do
  ...
  function F.Log.PrintWithArguments(parent, color, ...)
    ...
    -- detects the parent
    if parent == TYMEUI then
      stringHolder[2] = F.String.Good("(TYMEUI)")
    ...
    if not TYMEUI.DelayedWorldEntered then
